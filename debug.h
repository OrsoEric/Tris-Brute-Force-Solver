	...
	**********************************************************************************/
		...
		#define DSTOP()	\
			((_debug_file != NULL)?fprintf(_debug_file, "\nDebug has Ended!\n"), fflush(_debug_file), fclose(_debug_file), _debug_file = NULL, DSHOW(-99):(0))

		///----------------------------------------------------------------
		///	DEBUG PRINT MACROS
		///----------------------------------------------------------------

		//if show level == 0 user wants to see everything
		//if show level > 0 user wants to see nest level from 0 to showlevel (core calls)
		//if show level < 0 user wants to see nest level show level to infinity (edge calls)
		#define IS_DSHOW()	\
			(_debug_show_level > 0)?(_debug_indent_level <= _debug_show_level):(_debug_indent_level >= -_debug_show_level)

		//Print a given number of tab characters
		#define DTAB(n)	\
			(((_debug_file != NULL) && IS_DSHOW())?(fprintf(_debug_file,"%.*s", n, "\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t")):(0));

		//Print a number of tab equal to indent level than print user defined string
		#define DPRINT( ... )	\
			(((_debug_file != NULL) && IS_DSHOW())?(fprintf(_debug_file,"%.*s", _debug_indent_level, "\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t"), fprintf(_debug_file, __VA_ARGS__ )):(0))

		//print user defined string
		#define DPRINT_NOTAB( ... )	\
			(((_debug_file != NULL) && (IS_DSHOW()))?(fprintf(_debug_file, __VA_ARGS__ )):(0))
